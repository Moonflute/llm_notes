...
const frontiers = read("content/frontiers/index.json");
...
for (const title of ["Toolformer", "Claude 3", "DeepSeek-R1"]) fail(events.filter(event => event.title === title).length !== 1, `${title} must have exactly one canonical timeline event`);
...
for (const id of ["quantization", "reasoning", "fine-tuning", "mixture-of-experts", "inference-serving", "mla", "mcp"]) fail(!guideIds.has(id), `core concept is not a full study guide: ${id}`);
fail(guideIds.size < 25, "fewer than 25 full study guides");
const worldModels=frontiers.find(frontier=>frontier.slug==="world-models");
const interpretability=frontiers.find(frontier=>frontier.slug==="interpretability");
fail(!worldModels?.sourceIds.includes("world-models-paper"), "world models must cite the primary World Models paper");
fail(!interpretability?.sourceIds.includes("scaling-monosemanticity"), "interpretability must cite a directly relevant mechanistic interpretability source");
for(const frontier of [worldModels,interpretability])for(const id of frontier?.sourceIds??[])fail(["transformer","mamba"].includes(id),`${frontier.slug} has an irrelevant generic architecture source: ${id}`);
