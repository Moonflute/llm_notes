...
export default async function Release({ params }: { params: Promise<{ slug: string; release: string }> }) {
  ...
  return <main id="main-content" className="detail modelDetail">
    <Link href={"/models/"+family+"/"} className="back">← {modelFamily.titleKo} 계열</Link><p className="sectionLabel">모델 릴리스 · {item.date}</p><h1>{item.title}</h1><ContentMeta verifiedAt={profile.verifiedAt} />
