...
export async function generateMetadata({ params }: { params: Promise<{ slug: string; release: string }> }): Promise<Metadata> { const { slug, release } = await params; const item = modelReleases.find((entry) => entry.familySlug === slug && entry.slug === release); return { title: item?.title ?? "모델", description: item?.summary, alternates: { canonical: "/models/" + slug + "/" + release + "/" } }; }
...
export default async function Release({ params }: { params: Promise<{ slug: string; release: string }> }) {
  ...
  return <main id="main-content" className="detail modelDetail">
    <nav className="breadcrumb" aria-label="현재 위치"><Link href="/models/">모델</Link><span>›</span><Link href={"/models/"+family+"/"}>{modelFamily.titleKo}</Link><span>›</span><b>{item.title}</b></nav><Link href={"/models/"+family+"/"} className="back">← {modelFamily.titleKo} 계열로</Link><p className="sectionLabel">모델 릴리스 · {item.date}</p><h1>{item.title}</h1><ContentMeta verifiedAt={profile.verifiedAt} />
    ...
    <section><h2>발표 내용과 해석</h2>{profile.announcement.map((paragraph)=><p key={paragraph}>{paragraph}</p>)}<p>{profile.reception}</p></section>
    <section><h2>공개 스펙</h2><table className="specTable"><tbody>{specs.map(([label,value])=><tr key={label}><th scope="row">{label}</th><td>{value}</td></tr>)}</tbody></table><p className="specNote">공식 자료에 수치가 없으면 미공개로 표기합니다. 서로 다른 시점의 제품 한도와 논문·연구 설정은 혼용하지 않습니다.</p></section>
    ...
    <section><h2>공식 자료와 원문</h2>{item.sourceIds.map((id)=>{const source=getSource(id);return source?<a className="source" href={source.url} key={id} target="_blank" rel="noreferrer"><span>Tier {source.tier??1} · {source.publisher}</span><b>{source.title} ↗</b><small>{source.year} · 확인 {source.verifiedAt??"미기록"}</small></a>:null})}</section>
