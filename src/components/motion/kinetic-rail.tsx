...
type DragState = { pointerId: number; startX: number; startScroll: number; lastX: number; lastTime: number; velocity: number };
...
export function KineticRail({children,label,itemLabel="항목",className=""}:{children:ReactNode;label:string;itemLabel?:string;className?:string}){
  ...
  const begin=(event:PointerEvent<HTMLDivElement>)=>{
    ...
    drag.current={pointerId:event.pointerId,startX:event.clientX,startScroll:view.scrollLeft,lastX:event.clientX,lastTime:performance.now(),velocity:0};
  ...
  const wheel=(event:WheelEvent<HTMLDivElement>)=>{
    ...
    if(!delta)return;
    event.preventDefault();view.scrollLeft+=delta;
    window.clearTimeout(Number(view.dataset.snapTimer));
    view.dataset.snapTimer=String(window.setTimeout(snap,120));
  };

  return <section className={`kineticRail ${className}`} aria-label={label}>
    ...
    <div ref={viewport} className="kineticRailViewport" tabIndex={0} onPointerDown={begin} onPointerMove={move} onPointerUp={end} onPointerCancel={end} onWheel={wheel} onScroll={()=>{if(!drag.current)setActive(nearestIndex())}} onKeyDown={event=>{if(event.key==="ArrowLeft"){event.preventDefault();goTo(active-1)}if(event.key==="ArrowRight"){event.preventDefault();goTo(active+1)}}}>
