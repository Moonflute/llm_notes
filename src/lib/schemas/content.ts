...
export function validateReferences(sources:{id:string}[], events:{slug:string;sourceId:string;conceptIds?:string[]}[], concepts:{slug:string;sourceIds:string[];prerequisites:string[];next:string[]}[]) { const sourceIds=new Set(sources.map(s=>s.id)); const conceptIds=new Set(concepts.map(c=>c.slug)); for(const event of events) { if(!sourceIds.has(event.sourceId)) throw new Error(`Event ${event.slug} references missing source ${event.sourceId}`); for(const id of event.conceptIds??[]) if(!conceptIds.has(id)) throw new Error(`Event ${event.slug} references missing concept ${id}`); } for(const concept of concepts) { for(const id of concept.sourceIds) if(!sourceIds.has(id)) throw new Error(`Concept ${concept.slug} references missing source ${id}`); for(const slug of [...concept.prerequisites,...concept.next]) if(!conceptIds.has(slug)) throw new Error(`Concept ${concept.slug} references missing concept ${slug}`); } }
