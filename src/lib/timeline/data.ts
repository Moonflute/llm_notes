...
export const timelineNodes: TimelineNode[] = events.map(event => {
  ...
  return { id: event.id, slug: event.slug, entityType: entityType(event), titleKo: event.title, titleEn: event.titleEn, summaryKo: event.summary, primaryDate: historicalDate(event.date, [event.sourceId], event.type === "product" ? "product_released" : event.type === "model" ? "announced" : "paper_published"), additionalDates: [], importance: event.importance, laneId, subLaneId: familyId ? `family-${familyId}` : undefined, organizationIds: [organizationAliases[event.organization] ?? "other"], modelFamilyId: familyId, conceptIds: event.conceptIds, fieldIds: event.concepts.map(value => value.toLowerCase().replaceAll(" ", "-")), releaseMode: event.type === "product" ? "product" : event.type === "model" ? "paper" : undefined, sourceIds: [event.sourceId], status: "published" };
});
...
const nodeIdBySlug = new Map(timelineNodes.map(node => [node.slug, node.id]));
const conceptRelations = timelineNodes.flatMap(node => node.conceptIds.filter(id => ["transformer", "scaling-laws", "rlhf", "retrieval-augmented-generation", "agents", "reasoning"].includes(id)).map(id => {
  const anchorSlug = id === "transformer" ? "transformer" : id === "scaling-laws" ? "scaling-laws" : id === "rlhf" ? "instructgpt" : id === "retrieval-augmented-generation" ? "rag" : id === "agents" ? "tool-use" : "reasoning";
  return { id: `concept-${id}-${node.id}`, fromId: nodeIdBySlug.get(anchorSlug) ?? "", toId: node.id, type: "influenced_by" as const, evidence: "documented" as const, sourceIds: node.sourceIds, visibleFromZoom: 2 as const };
}).filter(relation => relation.fromId && relation.fromId !== relation.toId));
